...
namespace BusterWood.Caching
{
    public static class CacheExtensions
    {
        ...
        /// </summary>
        /// <param name="cache">The underlying cache to load data from</param>
        public static ThunderingHerdProtection<TKey, TValue> WithThunderingHerdProtection<TKey, TValue>(this IReadThroughCache<TKey, TValue> cache)
        {
        ...
        /// <param name="value">The value found, or default(T) if not found</param>
        /// <returns>TRUE if the item was found in the this cache or the underlying data source, FALSE if no item can be found</returns>
        public static bool TryGet<TKey, TValue>(this IReadThroughCache<TKey, TValue> cache, TKey key, out TValue value)
        {
            var maybe = cache.Get(key);
            value = maybe.GetValueOrDefault();
            return maybe.HasValue;
        ...
        /// <param name="key">The key to find</param>
        /// <returns>The value found, or default(T) if not found</returns>
        public static TValue GetValueOrDefault<TKey, TValue>(this IReadThroughCache<TKey, TValue> cache, TKey key)
        {
            return cache.Get(key).GetValueOrDefault();
        }
        ...
        /// <param name="key">The key to find</param>
        /// <returns>The value found, or default(T) if not found</returns>
        public static async Task<TValue> GetValueOrDefaultAsync<TKey, TValue>(this IReadThroughCache<TKey, TValue> cache, TKey key)
        {
        ...
        public static TValue[] GetBatchValueOrDefault<TKey, TValue>(this IReadThroughCache<TKey, TValue> cache, IReadOnlyCollection<TKey> keys)
        {
        ...
        /// <param name="keys">The keys to find</param>
        /// <returns>An array the same size as the input <paramref name="keys"/> that contains a value or default(T) for each key in the corresponding index</returns>
        public static async Task<TValue[]> GetBatchValueOfDefaultAsync<TKey, TValue>(this IReadThroughCache<TKey, TValue> cache, IReadOnlyCollection<TKey> keys)
        {
